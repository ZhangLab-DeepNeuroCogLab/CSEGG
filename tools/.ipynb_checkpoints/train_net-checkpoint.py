...
def stage_main(args, cfg, build):
    ...
    for i in range(num_tasks):
        ...
        if continual_method != "none" :
            ...
            path = f'{time_str}-{cfg.EXPERIMENT_NAME}{debug_flag}' +  "_TASK_" + str(task_number)
...
#             trainer.resume_or_load(resume=False, load_mapping=cfg.MODEL.WEIGHTS_LOAD_MAPPING)
        ...
        if comm.is_main_process() and cfg.MODEL.AS_PRETRAIN:
            ...
            if not (task_number == 1 and (continual_method == "ewc" or continual_method == "icarl")) :
...
if __name__ == "__main__":
    ...
    parser.add_argument("--continual", choices=["random_replay", "ewc", "icarl", False], required=False, default = False)

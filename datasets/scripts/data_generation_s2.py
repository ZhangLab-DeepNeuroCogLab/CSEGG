...
def convert_to_h5(dataset_dict, task_number, split_str, test_combined, exemp) :
    ...
    if split_str == "test" :
        ...
        if exemp == 10 :
            ...
            hf = h5py.File("s2_data/s2_task_" + str(task_number) + "_exemp_100" + "/" + "s2_task" + str(task_number) + ".h5", 'w')
        else :
...
def main(args):
    ...
    # s2(args.file_path, split="train")
    # s2(args.file_path, split="test")
    #s2(args.file_path, split="test", test_combined=True)
    ...
    # s2(args.file_path, split="train", test_combined=False, exemp=100)
